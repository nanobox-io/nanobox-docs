module ProdNavHelpers
  def prod_articles_in_order
    articles = []
  ...
  def prod_get_prev_article(current_article_path)
    articles = prod_articles_in_order
    index = prod_articles_in_order.find_index { |article| article[:path] == current_article_path }
  ...
  def prod_get_next_article(current_article_path)
    articles = prod_articles_in_order
    index = prod_articles_in_order.find_index { |article| article[:path] == current_article_path }
